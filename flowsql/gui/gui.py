...
def main():
    ...
    with dpg.window(label="flowsql", width=width, height=height):
        ...
        in_path = dpg.add_input_text(
            label="Path", default_value="unit-tests/customers.sql"
        )
        in_outputlocation = dpg.add_input_text(
            label="Output Location", default_value="outputs"
        )
